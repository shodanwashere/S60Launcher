...
@Composable
fun MenuStyleMenu(navController: NavController) {
    val context = LocalContext.current
    val menuItems = remember {
        listOf(
            R.string.drawer_style_grid, // Cambiar "Default" a "Grid"
            R.string.menu_style_list
        )
    }
    val menuStyles = remember {
        listOf(CustomAppInfoManager.MENU_STYLE_DEFAULT, CustomAppInfoManager.MENU_STYLE_LIST)
    }
    val currentMenuStyle = remember { CustomAppInfoManager.getMenuStyle(context) }
    ...
    LaunchedEffect(menuItems) {
        if (menuItems.isNotEmpty() && hasPhysicalKeyboard) {
            val initialFocusedIndex = menuStyles.indexOf(currentMenuStyle).coerceAtLeast(0)
    ...
    LazyColumn(
        ...
        itemsIndexed(menuItems) { index, itemRes ->
            ...
            val onClick = {
                CustomAppInfoManager.setMenuStyle(context, menuStyles[index])
                goBack()
            }
            ...
            Row(
                ...
                if (currentMenuStyle == menuStyles[index]) {
                    Icon(
